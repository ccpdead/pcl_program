...
main (int argc, char** argv)
{
  ...
  //体素滤波器,降低数据量
  pcl::VoxelGrid<pcl::PointXYZ> sor;//体素滤波
  sor.setInputCloud(cloud_pass_2);
  ...
  float x=0,y=0,z=0,nor_x=0,nor_y=0,nor_z=0;
    ...
    {	
      if(normals->points[i].normal_x <0.3 && normals->points[i].normal_x >-0.2)//筛选X方向
      {	
        if(normals->points[i].normal_z <-0.95 && normals->points[i].normal_z >-2.5)	//筛选Z方向
        {
          //将符合法线条件的点云保存在cloud_final中.
